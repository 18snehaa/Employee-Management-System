...
public class DataInitializer {
    ...
    CommandLineRunner initUsers(UserRepository repository, PasswordEncoder encoder) {
        return args -> {
            createOrReset(repository, encoder, "admin", "admin123", "ADMIN");
            createOrReset(repository, encoder, "user", "user123", "USER");
            System.out.println("EMS LOGIN USERS READY");
            System.out.println("ADMIN -> admin / admin123");
            System.out.println("USER  -> user / user123");
        };
    }

    private void createOrReset(UserRepository repository, PasswordEncoder encoder,
